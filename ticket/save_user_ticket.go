...
type SaveUserTicketRequest struct {
	core.BaseRequest
	// 企业ID
...
type SaveUserTicketResponse struct {
	core.BaseResponse
}

// 创建工单（客户）
// 调用此接口，系统会根据提交的参数按此优先级“客户 ID > 对接 ID > 客户邮箱 > 客户手机号”查找客户，如果匹配成功，用该客户作为工单关联客户，否则新建客户后关联。
func (t *Ticket) SaveUserTicket(ctx context.Context, req *SaveUserTicketRequest) (*SaveUserTicketResponse, error) {
	httpReq, err := t.httpClient.NewRawRequest(ctx, http.MethodPost, "/api/ws/5/ticket/save_user_ticket", req)
	if err != nil {
		return nil, err
	}

	var rsp SaveUserTicketResponse
